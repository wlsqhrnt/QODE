...
#define MAX_X 1960
#define MAX_Y 2710
#define RACKETSIZE (int)(9*MAX_X)/74
#define BALLSIZE (int)(5.5*MAX_X)/74
#define RACKETSECTOR (int)RACKETSIZE/15

int startCheck = 0;
volatile char send_buffer[24];
int startAngle;
...
volatile char single_mode_selection = 0; // 모드 싱글 모드 선택
volatile char PVP_mode_selection = 0; // 모드 대전 모드 선택
volatile char completion_mode_select = 0; // 모드 선택 완료
...
//B 통신 관련 카운트 변수
volatile char B_AR_readyCnt = 0;//라켓 B AR 증강 준비 완료
...
//AR 아이템 포지션 
long A_AR_item_pos1_X = MAX_X * 0.25;
long A_AR_item_pos2_X = MAX_X * 0.5;
long A_AR_item_pos3_X = MAX_X * 0.75;
long A_AR_item_pos_Y = MAX_Y * 0.15;

long B_AR_item_pos1_X = MAX_X * 0.75;
long B_AR_item_pos2_X = MAX_X * 0.5;
long B_AR_item_pos3_X = MAX_X * 0.25;
long B_AR_item_pos_Y = MAX_Y * 0.85;
//동전 투입
...
//16 -> 수비수 2
//32 -> 수비수 3
char A_changeUpCheck = 0;
char A_tornadoCheck = 0;
char A_mineCheck = 0;
char A_mineCnt = 0;
char A_defenderCheck = 0;
char A_defenderCnt = 0;

char B_tornado_stat = 0;
char B_tornadoCnt = 0;
char B_changeUpCheck = 0;
char B_tornadoCheck = 0;
char B_mineCheck = 0;
char B_mineCnt = 0;
char B_defenderCheck = 0;
char B_defenderCnt = 0;

char A_mineOn = 0;//지뢰 밟음 ㅎㅎ;;
char A_mineOnCnt = 0;//카운트 동안 공 멈춤 ㅎㅎ;;
...
void initTimer(void)//timer interrupt
{
    ...
    Timer5.attachInterrupt(A_mine_timerHandler).start(1000000);//1s
    Timer6.attachInterrupt(A_defender_timerHandler).start(1000000);//1s
    Timer7.attachInterrupt(B_mine_timerHandler).start(1000000);//1s
    Timer8.attachInterrupt(B_defender_timerHandler).start(1000000);//1s
}
void initPinConfiguration(void)
{
...
void limitSwitchCheck(void)
{
    ...
    if(digitalRead(A_X_limitSwitch2) == LOW && A_X_dir == 1){
        ...
        A_X_Cood = MAX_X; A_X_timerLimit = 3;
        A_X_timerCnt = 0;
    }
    ...
    if(digitalRead(B_X_limitSwitch2) == LOW && B_X_dir == 1){
        ...
        B_X_Cood = MAX_X; B_X_timerLimit = 3;
        B_X_timerCnt = 0;
    }
}
void ballMoveControl(void)
{
    ...
    if(ball_Y_timerCnt >= ball_Y_timerLimit && collision_check2 == 0
        ...
        ball_Y_timerCnt = 0;
        ball_Y_speedCheck++;
        if(ball_Y_timerLimit > 20 && ball_Y_speedCheck == 100){//가속 
            //ball_Y_timerLimit-=1;
            ball_Y_speedCheck = 0;
        }
    }
...
{   
    if(A_item == 1){ // fireball
        if(ball_Y_dir == 0){
            ...
            //y 축 좌표 비교해서 뜨거운거 레벨 조절
        }
    }
    if(B_item == 1){
        if(ball_Y_dir == 1){
            if(ball_Y_Cood > ((int)(MAX_Y * 0.34))){
                angle = 270;
                setAngle(angle,speed);
            }else if(ball_Y_Cood <= ((int)(MAX_Y * 0.34))){
                int randNum = rand() % 2;
                if(randNum == 0){
                    angle = rand() % 20 + 220;
                }else if(randNum == 1){
    ...
    if(A_item == 2){ // tornado
        if(ball_Y_dir == 0){
            if(ball_X_Cood < ((int)(MAX_X * 0.24))){
                if(ball_Y_Cood < ((int)(MAX_Y * 0.18)) && A_tornadoCheck == 0){
                    A_tornadoCheck++;
                    angle = 90;
                    setAngle(angle,speed);
                }
                if(ball_Y_Cood >= ((int)(MAX_Y * 0.18)) && A_tornadoCheck == 1){
                    A_tornadoCheck++;
                    angle = 80;
                    setAngle(angle,speed);
                }
                if(ball_Y_Cood >= ((int)(MAX_Y * 0.36)) && A_tornadoCheck == 2){//휘기 시작
            ...
            else if(((int)(MAX_X * 0.24))<= ball_X_Cood && ball_X_Cood < ((int)(MAX_X * 0.42))){
                if(ball_Y_Cood < ((int)(MAX_Y * 0.18)) && A_tornadoCheck == 0){
                    A_tornadoCheck++;
                    angle = 110;
                    setAngle(angle,speed);
            ...
            else if(((int)(MAX_X * 0.42)) <= ball_X_Cood && ball_X_Cood < ((int)(MAX_X * 0.74))){
                if(ball_Y_Cood < ((int)(MAX_Y * 0.18)) && A_tornadoCheck == 0){
                    A_tornadoCheck++;
                    angle = 70;
                    setAngle(angle,speed);
            ...
            else if(ball_X_Cood >= ((int)(MAX_X * 0.74))){
                if(ball_Y_Cood < ((int)(MAX_Y * 0.18)) && A_tornadoCheck == 0){
                    A_tornadoCheck++;
                    angle = 90;
                    setAngle(angle,speed);
            ...
            //y 축 좌표 비교해서 차가운거 레벨 조절
        }
    }
    if(B_item == 2){
        if(ball_Y_dir == 1){
            if(ball_X_Cood < ((int)(MAX_X * 0.24))){ 
                if(ball_Y_Cood > ((int)(MAX_Y * 0.74)) && B_tornadoCheck == 0){
                    B_tornadoCheck++;
                    angle = 270;
                    setAngle(angle,speed);
                }
                if(ball_Y_Cood <= ((int)(MAX_Y * 0.74)) && B_tornadoCheck == 1){
                    B_tornadoCheck++;
                    angle = 280;
                    setAngle(angle,speed);
                }
                if(ball_Y_Cood <= ((int)(MAX_Y * 0.41)) && B_tornadoCheck == 2){//휘기 시작
    ...
    if(A_item == 3){ // change up
        if(A_changeUpCheck == 0) 
            A_changeUpCheck+=1;
        if(ball_Y_dir == 0){
            if(( ball_Y_Cood == ((int)(MAX_Y * 0.11)) && A_changeUpCheck == 1) ||
                (ball_Y_Cood == ((int)(MAX_Y * 0.34)) && A_changeUpCheck == 2) ||
                (ball_Y_Cood == ((int)(MAX_Y * 0.46)) && A_changeUpCheck == 3) ||
                (ball_Y_Cood == ((int)(MAX_Y * 0.57)) && A_changeUpCheck == 4) ){
                    if(ball_X_dir == 1)
    ...
    if(B_item == 3){
        if(B_changeUpCheck == 0) 
            B_changeUpCheck+=1;
        if(ball_Y_dir == 1){
            if((ball_Y_Cood == ((int)(MAX_Y * 0.81)) && B_changeUpCheck == 1) ||
                (ball_Y_Cood == ((int)(MAX_Y * 0.57)) && B_changeUpCheck == 2) ||
                (ball_Y_Cood == ((int)(MAX_Y * 0.46)) && B_changeUpCheck == 3) ||
                (ball_Y_Cood == ((int)(MAX_Y * 0.34)) && B_changeUpCheck == 4) ){
                    if(ball_X_dir == 1)
...
void ARitemControl(void)
{
    if(A_mineCheck == 1 && ball_Y_dir == 1){
        if(ball_Y_Cood == A_AR_item_pos_Y){
            if(((A_AR_item & 1) == 1) && A_AR_item_pos1_X - (long)(MAX_X * 0.03125) <= ball_X_Cood && 
               ball_X_Cood <= A_AR_item_pos1_X + (long)(MAX_X * 0.03125)){
                Serial2.write('u');
                A_mineOn  = 1;
                A_AR_item -= 1;
            }
            else if(((A_AR_item & 2) == 2) && A_AR_item_pos2_X - (long)(MAX_X * 0.03125) <= ball_X_Cood && 
               ball_X_Cood <= A_AR_item_pos2_X + (long)(MAX_X * 0.03125)){
                Serial2.write('v');
                A_mineOn  = 1;
                A_AR_item -= 2;
            }
            else if(((A_AR_item & 4) == 4) && A_AR_item_pos3_X - (long)(MAX_X * 0.03125) <= ball_X_Cood && 
               ball_X_Cood <= A_AR_item_pos3_X + (long)(MAX_X * 0.03125)){
                Serial2.write('w');
                A_mineOn  = 1;
                A_AR_item -= 4;
            }
    ...
    if(A_defenderCheck == 1 && ball_Y_dir == 1){
        if(ball_Y_Cood == A_AR_item_pos_Y){
            if(((A_AR_item & 8) == 8) && A_AR_item_pos1_X - (long)(MAX_X * 0.03125) <= ball_X_Cood
            && ball_X_Cood <= A_AR_item_pos1_X + (long)(MAX_X * 0.03125))
            {   
                Serial2.write('x');
                if(A_hacking_check == 1)
            ...
            else if(((A_AR_item & 16) == 16) && A_AR_item_pos2_X - (long)(MAX_X * 0.03125) <= ball_X_Cood
            && ball_X_Cood <= A_AR_item_pos2_X + (long)(MAX_X * 0.03125))
            {   
                Serial2.write('y');
                if(A_hacking_check == 1)
            ...
            else if(((A_AR_item & 32) == 32) && A_AR_item_pos3_X - (long)(MAX_X * 0.03125) <= ball_X_Cood
            && ball_X_Cood <= A_AR_item_pos3_X + (long)(MAX_X * 0.03125))
            {
                Serial2.write('z');
                if(A_hacking_check == 1)
    ...
    if(B_mineCheck == 1 && ball_Y_dir == 0){
        if(ball_Y_Cood == A_AR_item_pos_Y){
            if(((B_AR_item & 1) == 1) && B_AR_item_pos1_X - (long)(MAX_X * 0.03125) <= ball_X_Cood && 
               ball_X_Cood <= A_AR_item_pos1_X + (long)(MAX_X * 0.03125)){
                Serial3.write('u');
                B_mineOn  = 1;
                B_AR_item -= 1;
            }
            else if(((B_AR_item & 2) == 2) && B_AR_item_pos2_X - (long)(MAX_X * 0.03125) <= ball_X_Cood && 
               ball_X_Cood <= A_AR_item_pos2_X + (long)(MAX_X * 0.03125)){
                Serial3.write('v');
                B_mineOn  = 1;
                B_AR_item -= 2;
            }
            else if(((B_AR_item & 4) == 4) && B_AR_item_pos3_X - (long)(MAX_X * 0.03125) <= ball_X_Cood && 
               ball_X_Cood <= A_AR_item_pos3_X + (long)(MAX_X * 0.03125)){
                Serial3.write('w');
                B_mineOn  = 1;
                B_AR_item -= 4;
    ...
    if(B_defenderCheck == 1 && ball_Y_dir == 0){
        if(ball_Y_Cood == B_AR_item_pos_Y){
            if(((B_AR_item & 8) == 8) && B_AR_item_pos1_X - (long)(MAX_X * 0.03125) <= ball_X_Cood
            && ball_X_Cood <= B_AR_item_pos3_X + (long)(MAX_X * 0.03125))
            {   
                Serial3.write('x');
                if(B_hacking_check == 1)
                    B_hacking_check = 0;
            ...
            else if(((B_AR_item & 16) == 16) && B_AR_item_pos2_X - (long)(MAX_X * 0.03125) <= ball_X_Cood
            && ball_X_Cood <= B_AR_item_pos2_X + (long)(MAX_X * 0.03125))
            {
                Serial3.write('y');
                if(B_hacking_check == 1)
                    B_hacking_check = 0;
            ...
            else if(((B_AR_item & 32) == 32) && B_AR_item_pos3_X - (long)(MAX_X * 0.03125) <= ball_X_Cood
            && ball_X_Cood <= B_AR_item_pos1_X + (long)(MAX_X * 0.03125))
            {
                Serial3.write('z');
                if(B_hacking_check == 1)
                    B_hacking_check = 0;
    ...
    if(A_mineCnt == 5){
        ...
        A_mineCheck = 0;
        if(A_AR_item & 1 == 1){
            Serial2.write('A');//아이템 1 위치 소멸
            A_AR_item -= 1;
        }else if(A_AR_item & 2 == 2){
            Serial2.write('B');//아이템 2 위치 소멸
            A_AR_item -= 2;
        }else if(A_AR_item & 4 == 4){
            Serial2.wirte('C');//아이템 3 위치 소멸
            A_AR_item -= 4;
        }
    }
    if(A_defenderCnt == 5){
        ...
        if(A_AR_item & 8 == 8){
            Serial2.write('A');//아이템 1 위치 소멸
            A_AR_item -= 8;
        }else if(A_AR_item & 16 == 16){
            Serial2.write('B');//아이템 2 위치 소멸
            A_AR_item -= 16;
        }else if(A_AR_item & 32 == 32){
            Serial2.wirte('C');//아이템 3 위치 소멸
            A_AR_item -= 32;
        }
    }
    if(B_mineCnt == 5){
        ...
        B_defenderCheck = 0;
        if(B_AR_item & 1 == 1){
            Serial3.write('A');//아이템 1 위치 소멸
            B_AR_item -= 1;
        }else if(B_AR_item & 2 == 2){
            Serial3.write('B');//아이템 2 위치 소멸
            B_AR_item -= 2;
        }else if(B_AR_item & 4 == 4){
            Serial3.wirte('C');//아이템 3 위치 소멸
            B_AR_item -= 4;
        }
    }
    if(B_defenderCnt == 5){
        ...
        if(B_AR_item & 8 == 8){
            Serial3.write('A');//아이템 1 위치 소멸
            B_AR_item -= 8;
        }else if(B_AR_item & 16 == 16){
            Serial3.write('B');//아이템 2 위치 소멸
            B_AR_item -= 16;
        }else if(B_AR_item & 32 == 32){
            Serial3.wirte('C');//아이템 3 위치 소멸
            B_AR_item -= 32;
        }
    }
...
void initing(void)
{
    ...
    for (int i = 0; i < ((int)(MAX_Y/2) - (int)(MAX_X/2)) ; i++) {
        digitalWrite(ball_Y_stpPin, HIGH);
        delayMicroseconds(400);
        digitalWrite(ball_Y_stpPin, LOW);
        delayMicroseconds(400);
...
void A_defender_timerHandler(void)
{
    if(A_defenderCheck == 1){
        A_defenderCnt++;
    }
}
void B_mine_timerHandler(void)
{
    if(B_mineCheck == 1){
        B_mineCnt++;
    }
...
void B_defender_timerHandler(void)
{
    if(B_defenderCheck == 1){
        B_defenderCnt++;
    }
}
//***************************************************************************************************************//
...
void serialEvent2(){
    if((A_bluetooth_data = Serial2.read()) != -1){ //스마트폰 -> 아두이노 -> PC
        switch(A_bluetooth_data){
            ...
            case 'B' : PVP_mode_selection = 1; break;// 대전 모드 선택
            case 'C' : single_mode_selection = 1; break;// 싱글 모드 선택
            case 'D' : completion_mode_select = 1; break;
            case 'E' : A_pre_game_selected = 1; break;// 선공 게임 선택
            case 'F' : A_AR_ready = 1; break;// AR 준비 완료 
            case 'G' : A_moveCtl = 1; A_X_timerLimit = 7; A_change_left_dir(); break;//왼쪽으로 1속도 이동
            case 'H' : A_moveCtl = 2; A_X_timerLimit = 6; A_change_left_dir(); break;//왼쪽으로 2속도 이동
            case 'I' : A_moveCtl = 3; A_X_timerLimit = 5; A_change_left_dir(); break;//왼쪽으로 3속도 이동
            case 'J' : A_moveCtl = 4; A_X_timerLimit = 4; A_change_left_dir(); break;//왼쪽으로 4속도 이동
            case 'K' : A_moveCtl = 5; A_X_timerLimit = 3; A_change_left_dir(); break;//왼쪽으로 5속도 이동
            case 'L' : A_moveCtl = 0;  A_move_stop = 1; break;// 정지
            case 'M' : A_moveCtl = 1; A_X_timerLimit = 7; A_change_right_dir(); break;//오른쪽으로 1속도 이동
            case 'N' : A_moveCtl = 2; A_X_timerLimit = 6; A_change_right_dir(); break;//오른쪽으로 2속도 이동
            case 'O' : A_moveCtl = 3; A_X_timerLimit = 5; A_change_right_dir(); break;//오른쪽으로 3속도 이동
            case 'P' : A_moveCtl = 4; A_X_timerLimit = 4; A_change_right_dir(); break;//오른쪽으로 4속도 이동
            case 'Q' : A_moveCtl = 5; A_X_timerLimit = 3; A_change_right_dir(); break;//오른쪽으로 5속도 이동
            case 'R' : A_get_mine = 1; break;//지뢰 아이템 획득 
            case 'S' : A_get_defender = 1; break;//수비수 아이템 획득
            case 'T' : A_use_mine_1 = 1;  A_mineCheck = 1; A_AR_item += 1; break; //라켓 A 지뢰 1위치 사용
            case 'U' : A_use_mine_2 = 1; A_mineCheck = 1; A_AR_item += 2; break; //라켓 A 지뢰 2위치 사용
            case 'V' : A_use_mine_3 = 1; A_mineCheck = 1; A_AR_item += 4;break; //라켓 A 지뢰 3위치 사용
            case 'W' : A_use_defender_1 = 1; A_defenderCheck = 1; A_AR_item += 8; break; //라켓 A 수비수 1위치 사용
            case 'X' : A_use_defender_2 = 1; A_defenderCheck = 1; A_AR_item += 16; break; //라켓 A 수비수 2위치 사용
            case 'Y' : A_use_defender_3 = 1; A_defenderCheck = 1; A_AR_item += 32; break; //라켓 A 수비수 3위치 사용
            case 'Z' : A_use_fireball = 1; break; //파이어볼사용
            case 'a' : A_use_tornado = 1; break;//토네이도 사용
            case 'b' : A_use_hacking = 1; break;//해킹 사용
            case 'c' : A_use_EMP = 1; break;//EMP 사용
            case 'd' : A_use_changeup = 1; break;//CHANGE UP 사용
            case 'e' : A_use_iceball = 1; break;//아이스볼 사용
        }
    }
}
void serialEvent3(){
    if((B_bluetooth_data = Serial3.read()) != -1){ //스마트폰 -> 아두이노 -> PC
        switch(B_bluetooth_data){
            ...
            case 'C' : single_mode_selection = 1; break;// 싱글 모드 선택
            case 'D' : completion_mode_select = 1; break;
            case 'E' : B_pre_game_selected = 1; break;// 선공 게임 선택
            case 'F' : B_AR_ready = 1; break;// AR 준비 완료 
            case 'G' : B_moveCtl = 1; B_X_timerLimit = 7; B_change_left_dir(); break;//왼쪽으로 1속도 이동
            case 'H' : B_moveCtl = 2; B_X_timerLimit = 6; B_change_left_dir(); break;//왼쪽으로 2속도 이동
            case 'I' : B_moveCtl = 3; B_X_timerLimit = 5; B_change_left_dir(); break;//왼쪽으로 3속도 이동
            case 'J' : B_moveCtl = 4; B_X_timerLimit = 4; B_change_left_dir(); break;//왼쪽으로 4속도 이동
            case 'K' : B_moveCtl = 5; B_X_timerLimit = 3; B_change_left_dir(); break;//왼쪽으로 5속도 이동
            case 'L' : B_moveCtl = 0;  B_move_stop = 1; break;// 정지
            case 'M' : B_moveCtl = 1; B_X_timerLimit = 7; B_change_right_dir(); break;//오른쪽으로 1속도 이동
            case 'N' : B_moveCtl = 2; B_X_timerLimit = 6; B_change_right_dir(); break;//오른쪽으로 2속도 이동
            case 'O' : B_moveCtl = 3; B_X_timerLimit = 5; B_change_right_dir(); break;//오른쪽으로 3속도 이동
            case 'P' : B_moveCtl = 4; B_X_timerLimit = 4; B_change_right_dir(); break;//오른쪽으로 4속도 이동
            case 'Q' : B_moveCtl = 5; B_X_timerLimit = 3; B_change_right_dir(); break;//오른쪽으로 5속도 이동
            case 'R' : B_get_mine = 1; break; //지뢰 아이템 획득 
            case 'S' : B_get_defender = 1; break; //수비수 아이템 획득
            case 'T' : B_use_mine_1 = 1; B_mineCheck = 1; B_AR_item += 1; break; //라켓 B 지뢰 1위치 사용
            case 'U' : B_use_mine_2 = 1; B_mineCheck = 1; B_AR_item += 2; break; //라켓 B 지뢰 2위치 사용
            case 'V' : B_use_mine_3 = 1; B_mineCheck = 1; B_AR_item += 4; break; //라켓 B 지뢰 3위치 사용
            case 'W' : B_use_defender_1 = 1; B_defenderCheck = 1; B_AR_item += 8; break; //라켓 B 수비수 1위치 사용
            case 'X' : B_use_defender_2 = 1; B_defenderCheck = 1; B_AR_item += 16; break; //라켓 B 수비수 2위치 사용
            case 'Y' : B_use_defender_3 = 1; B_defenderCheck = 1; B_AR_item += 32; break; //라켓 B 수비수 3위치 사용
            case 'Z' : B_use_fireball = 1; break; //파이어볼사용
            case 'a' : B_use_tornado = 1; break;//토네이도 사용
            case 'b' : B_use_hacking = 1; break;//해킹 사용
            case 'c' : B_use_EMP = 1; break;//EMP 사용
            case 'd' : B_use_changeup = 1; break;//CHANGE UP 사용
            case 'e' : B_use_iceball = 1; break;//아이스볼 사용
        }
    }
}
//***************************************************************************************************************//
...
void serial_send_data(void)
{
    ...
    send_buffer[24] = checkSum ^ 0xFF;//check sum
    Serial1.write(send_buffer[24]);
}
void ball_X_Cood_function(void)
...
{//11번째 데이터 A 플레이어가 블루투스로 통해 보낸 신호 처리
  char game_ready = 0;
    ...
    if(completion_mode_select == 1 || completion_mode_selectCnt != 0){
      completion_mode_select = 0;
      game_ready += 16;
      completion_mode_selectCnt += 1;
      if(completion_mode_selectCnt == 3)
        completion_mode_selectCnt = 0;
    }
...
char A_activate_or_delete_item_function(void)
{
  ...
  if(A_activated_mine_3 == 1 || A_activated_mine_3Cnt != 0){
    ...
    A_activated_mine_3Cnt += 1;
    if(A_activated_mine_3Cnt == 3)
      A_activated_mine_3Cnt = 0;
  }
...
char B_activate_or_delete_item_function(void)
{
  ...
  if(B_activated_mine_3 == 1 || B_activated_mine_3Cnt != 0){
    ...
    B_activated_mine_3Cnt += 1;
    if(B_activated_mine_3Cnt == 3)
      B_activated_mine_3Cnt = 0;
  }
...
{//8번째 데이터 A 아이템 사용
    if(new_receive_buffer[4] != 0){
        if((new_receive_buffer[4] & 1) == 1 && (old_receive_buffer[4] & 1)== 0) Serial2.write('k');//파이어볼 사용
        if((new_receive_buffer[4] & 2) == 2 && (old_receive_buffer[4] & 2)== 0) Serial2.write('l');//토네이도 사용
        if((new_receive_buffer[4] & 4) == 4 && (old_receive_buffer[4] & 4)== 0){ B_hacking_check = 1; Serial2.write('m');}//해킹 사용
        if((new_receive_buffer[4] & 8) == 8 && (old_receive_buffer[4] & 8)== 0) Serial2.write('n');//EMP 사용
        if((new_receive_buffer[4] & 16) == 16 && (old_receive_buffer[4] & 16)== 0) Serial2.write('o');//체인지업 사용
        if((new_receive_buffer[4] & 32) == 32 && (old_receive_buffer[4] & 32)== 0) Serial2.write('p');//아이스볼 사용 
    }
}
void A_get_item(void)
{//7번째 데이터
    if(new_receive_buffer[5] != 0){
        ...
        if((new_receive_buffer[5] & 2 ) == 2 && (old_receive_buffer[5] & 2)== 0)  Serial2.write('f');// 토네이도 획득
        if((new_receive_buffer[5] & 4 ) == 4 && (old_receive_buffer[5] & 4)== 0) Serial2.write('g');// 해킹 획득
        if((new_receive_buffer[5] & 8 ) == 8 && (old_receive_buffer[5] & 8)== 0) Serial2.write('h');//EMP 획득
        if((new_receive_buffer[5] & 16 ) == 16 && (old_receive_buffer[5] & 16)== 0) Serial2.write('i');//change up 획득
        if((new_receive_buffer[5] & 32) == 32 && (old_receive_buffer[5] & 32)== 0) Serial2.write('j');//아이스볼 획득
    }
}
void completion_choose(void)
{//6번째 데이터
    if(new_receive_buffer[6] != 0){
        if((new_receive_buffer[6] & 1) == 1 && (old_receive_buffer[6] & 1)== 0){
          ...
          Serial2.write('b');//모드 선택 완료
          Serial3.write('b');//모드 선택 완료
        } 
        else if((new_receive_buffer[6] & 4) == 4 && (old_receive_buffer[6] & 4)== 0){
          Serial3.write('c');//선공 게임 선택 완료
          Serial3.write('c');//선공 게임 선택 완료
        } 
        else if((new_receive_buffer[6] & 8) == 8 && (old_receive_buffer[6] & 8)== 0){
          Serial2.write('d');//증강 완료
          Serial3.write('d');//증강 완료
        } 
    }
}
void B_using_item(void)
{//5번째 데이터
    if(new_receive_buffer[7] != 0){
        if((new_receive_buffer[7] & 1) == 1 && (old_receive_buffer[7] & 1)== 0) Serial3.write('h');//파이어볼 사용
        else if((new_receive_buffer[7] & 2) == 2 && (old_receive_buffer[7] & 2)== 0) Serial3.write('l');//토네이도 사용
        else if((new_receive_buffer[7] & 4) == 4 && (old_receive_buffer[7] & 4)== 0){ A_hacking_check = 1; Serial3.write('m');}//해킹 사용
        else if((new_receive_buffer[7] & 8) == 8 && (old_receive_buffer[7] & 8)== 0)  Serial3.write('n');// EMP 사용
        else if((new_receive_buffer[7] & 16) == 16 && (old_receive_buffer[7] & 16)== 0) Serial3.write('o');// 체인지업 사용
        else if((new_receive_buffer[7] & 32) == 32 && (old_receive_buffer[7] & 32)== 0) Serial3.write('p');//아이스볼 사용
    }
}
void B_get_item(void)
{//4번째 데이터
    if(new_receive_buffer[8] != 0){
        ...
        else if((new_receive_buffer[8] & 32 ) == 32 && (old_receive_buffer[8] & 32)== 0) Serial3.write('j');//아이스볼  획득
    }
}
//******************************************************************************************************************//
//*********************************************** The others function **********************************************//
