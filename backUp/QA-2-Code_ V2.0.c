...
void loop(){
    if(firstCheck == 0){// 선공게임이 정해지지 않았을때 ( 게임 시작 전 )
        ...
        if(digitalRead(A_X_limitSwitch2) == LOW && A_X_dir == 1){
            ...
            A_X_Cood = 1154; A_X_timerLimit = 8;
            A_X_timerCnt = 0;
        }
        ...
        if(digitalRead(B_X_limitSwitch2) == LOW && B_X_dir == 0){
            ...
            B_X_Cood = 1154; B_X_timerLimit = 8;
            B_X_timerCnt = 0;
        }
        //move
        ...
        if(A_moveCtl != 0){  // Racket A motor
            if(A_X_timerCnt >= A_X_timerLimit){
                ...
                A_X_timerCnt = 0;
                A_X_speedCheck++;
                if(A_X_timerLimit > 6 && A_X_stpPin == 100){
                    A_X_timerLimit-=1;
                    A_X_speedCheck = 0;
                }
            }
        ...
        if(B_moveCtl != 0){ // Racket B Motor
            if(B_X_timerCnt >= B_X_timerLimit){
                ...
                B_X_timerCnt = 0;
                B_X_speedCheck++;
                if(B_X_timerLimit > 6 && B_X_speedCheck == 100){
                    B_X_timerLimit-=1;
                    B_X_speedCheck = 0;
                }
            }
        ...
        if(item == 1){ // fireball
            ...
            }else if(ball_Y_dir == 1){
                if(ball_Y_Cood > 478){
                    angle = 240;
                    setAngle(angle,speed);
                }else if(ball_Y_Cood <= 478){
                    angle = rand() % 100 + 220;
                    setAngle(angle,speed);
                    item = 0;
                }
        ...
        if(item == 2){ // tornado
            if(ball_Y_dir == 0){
                if(ball_X_Cood < 278){
                    if(ball_Y_Cood < 250 && tornadoCheck == 0){
                        tornadoCheck++;
                        angle = 90;
                        setAngle(angle,speed);
                    }
                    if(ball_Y_Cood >= 250 && tornadoCheck == 1){
                        tornadoCheck++;
                        angle = 80;
                        setAngle(angle,speed);
                    }
                    if(ball_Y_Cood >= 600 && tornadoCheck == 2){//휘기 시작
                        if(tornadoCnt >= 10 && angle > 40){
                ...
                else if(278 <= ball_X_Cood && ball_X_Cood < 577){
                    if(ball_Y_Cood < 250 && tornadoCheck == 0){
                        tornadoCheck++;
                        angle = 110;
                        setAngle(angle,speed);
                ...
                else if(577 <= ball_X_Cood && ball_X_Cood < 855){
                    if(ball_Y_Cood < 250 && tornadoCheck == 0){
                        tornadoCheck++;
                        angle = 70;
                        setAngle(angle,speed);
                ...
                if(ball_X_Cood < 278){ 
                    if(ball_Y_Cood > 1023 && tornadoCheck == 0){
                        tornadoCheck++;
                        angle = 270;
                        setAngle(angle,speed);
                    }
                    if(ball_Y_Cood <= 1023 && tornadoCheck == 1){
                        tornadoCheck++;
                        angle = 280;
                        setAngle(angle,speed);
                    }
                    if(ball_Y_Cood <= 673 && tornadoCheck == 2){//휘기 시작
                        if(tornadoCnt >= 10 && angle < 320){
                        //tornadoCnt 크면 각 휘는 빈도수 증가, Anlge 최종적 휘는 정도
                            angle += 1;
                            tornadoCnt = 0;
                            setAngle(angle,speed);
        ...
        if(item == 3){ // change up
            if(changeUpCheck == 0) 
                changeUpCheck+=1;
            if(ball_Y_dir == 0){
                if(( ball_Y_Cood == 159 && changeUpCheck == 1) ||
                    (ball_Y_Cood == 477 && changeUpCheck == 2) ||
                    (ball_Y_Cood == 636 && changeUpCheck == 3) ||
                    (ball_Y_Cood == 795 && changeUpCheck == 4) ){
                        if(ball_X_dir == 0)
                ...
                if((ball_Y_Cood == 1113 && changeUpCheck == 1) ||
                    (ball_Y_Cood == 795 && changeUpCheck == 2) ||
                    (ball_Y_Cood == 636 && changeUpCheck == 3) ||
                    (ball_Y_Cood == 477 && changeUpCheck == 4) ){
                        if(ball_X_dir == 0)
...
void initing(){
    ...
    ball_X_Cood = 1154; ball_Y_Cood=0; A_X_Cood = 0; B_X_Cood = 0;
...
void serialEvent2(){
    if((A_bluetooth_data = Serial2.read()) != -1){ //스마트폰 -> 아두이노 -> PC
        switch(A_bluetooth_data){
            ...
            case '!' : A_moveCtl = 1; A_change_left_dir(); break;//왼쪽으로 1속도 이동
            case '"' : A_moveCtl = 2; A_change_left_dir(); break;//왼쪽으로 2속도 이동
            case '#' : A_moveCtl = 3; A_change_left_dir(); break;//왼쪽으로 3속도 이동
            case '$' : A_moveCtl = 4; A_change_left_dir(); break;//왼쪽으로 4속도 이동
            case '&' : A_moveCtl = 5; A_change_left_dir(); break;//왼쪽으로 5속도 이동
            case '(' : A_moveCtl = 0;  A_move_stop = 1; break;// 정지
            case 'Z' : A_moveCtl = 1; A_change_right_dir(); break;//오른쪽으로 1속도 이동
            case 'a' : A_moveCtl = 2; A_change_right_dir(); break;//오른쪽으로 2속도 이동
            case 'b' : A_moveCtl = 3; A_change_right_dir(); break;//오른쪽으로 3속도 이동
            case 'c' : A_moveCtl = 4; A_change_right_dir(); break;//오른쪽으로 4속도 이동
            case 'd' : A_moveCtl = 5; A_change_right_dir(); break;//오른쪽으로 5속도 이동
...
void serialEvent3(){
    if((B_bluetooth_data = Serial3.read()) != -1){ //스마트폰 -> 아두이노 -> PC
        switch(B_bluetooth_data){
            ...
            case '!' : B_moveCtl = 1; B_change_left_dir(); break;//왼쪽으로 1속도 이동
            case '"' : B_moveCtl = 2; B_change_left_dir(); break;//왼쪽으로 2속도 이동
            case '#' : B_moveCtl = 3; B_change_left_dir(); break;//왼쪽으로 3속도 이동
            case '$' : B_moveCtl = 4; B_change_left_dir(); break;//왼쪽으로 4속도 이동
            case '&' : B_moveCtl = 5; B_change_left_dir(); break;//왼쪽으로 5속도 이동
            case '(' : B_moveCtl = 0; B_move_stop = 1; break;// 정지
            case 'Z' : B_moveCtl = 1; B_change_right_dir(); break;//오른쪽으로 1속도 이동
            case 'a' : B_moveCtl = 2; B_change_right_dir(); break;//오른쪽으로 2속도 이동
            case 'b' : B_moveCtl = 3; B_change_right_dir(); break;//오른쪽으로 3속도 이동
            case 'c' : B_moveCtl = 4; B_change_right_dir(); break;//오른쪽으로 4속도 이동
            case 'd' : B_moveCtl = 5; B_change_right_dir(); break;//오른쪽으로 5속도 이동
...
void serial_send_data(){
    ...
    send_buffer[21] = 0x5B;
    send_buffer[20] = 4;//CMD
    send_buffer[19] = ball_X_Cood_High;
    send_buffer[18] = ball_X_Cood_Low;
    send_buffer[17] = ball_Y_Cood_High;
    send_buffer[16] = ball_Y_Cood_Low;
    send_buffer[15] = ball_check;
    send_buffer[14] = A_X_Cood_High;
    send_buffer[13] = A_X_Cood_Low;
    send_buffer[12] = A_game_ready;
    send_buffer[11] = A_use_item;
    send_buffer[10] = A_move;
    send_buffer[9] = B_X_Cood_High;
    send_buffer[8] = B_X_Cood_Low;
    send_buffer[7] = B_game_ready;
    send_buffer[6] = B_use_item;
    send_buffer[5] = B_move;
    send_buffer[4] = 0;//0x1;//insert_coin;
    send_buffer[3] = 0;//14;//게임 전 설정
    send_buffer[2] = 0;//예비
    send_buffer[1] = 0x5C;//end
...
void A_game_ready_function(){//11번째 데이터 A 플레이어가 블루투스로 통해 보낸 신호 처리
    if(A_pre_game_selected == 1){//선공게임 완료
        A_game_ready += 1;
    } 
...
void B_game_ready_function(){//6번째 데이터
    if(B_pre_game_selected == 1 ){//선공 게임 완료  
        B_game_ready += 1;
    } 
...
void ball_move_item(){//11번째 데이터
    if((new_receive_buffer[1] & 1)== 1 && (old_receive_buffer[1] & 1)== 0) item = 1;
    else if((new_receive_buffer[1] & 2) == 2 && (old_receive_buffer[1] & 2)== 0) item = 2;
    else if((new_receive_buffer[1] & 4) == 4 && (old_receive_buffer[1] & 4)== 0) item = 3;
}
...
void ball_stat(){//9번째 데이터
   ...
   /* else if((new_receive_buffer[3] & 4) == 4 && (old_receive_buffer[3] & 4) == 0){ // 골인 
        if(ball_Y_Cood < 500){ //B 득점 A led
            digitalWrite(led_bottom_pin,OUTPUT);
        }else if(ball_Y_Cood > 500){//A 득점 B led
            digitalWrite(led_top_pin,OUTPUT);
        }
        delay(3500);
        digitalWrite(led_bottom_pin,LOW);
        digitalWrite(led_top_pin,LOW);
    ...
    if((new_receive_buffer[3] & 16) == 16 && (old_receive_buffer[3] & 16) == 0){ //게임 재시작
        collision_check2 = 0;
        digitalWrite(led_bottom_pin,LOW);
        digitalWrite(led_top_pin,LOW);

    }
    else if((new_receive_buffer[3] & 8) == 8 && (old_receive_buffer[3] & 8)== 0){//게임 종료
        firstAt = 0;
        startCheck = 0;
        firstCheck = 0;
        ball_X_dir = 0;
...
void A_Collision_check(){
    if(ball_X_Cood < A_X_Cood){//라켓의 왼쪽
        if(A_X_Cood - 10 <= ball_X_Cood && ball_X_Cood <= A_X_Cood){//4번
            angle = 90;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(A_X_Cood - 30 <= ball_X_Cood && ball_X_Cood <= A_X_Cood - 10){//3번
            angle = 110;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(A_X_Cood - 50 <= ball_X_Cood && ball_X_Cood <= A_X_Cood - 30){//2번
            angle = 125;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(A_X_Cood - 70 <= ball_X_Cood && ball_X_Cood <= A_X_Cood - 50){//1번
            angle = 140;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(ball_X_Cood + 36 >= A_X_Cood - 70){//1번
            angle = 140;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        ...
        if(A_X_Cood <= ball_X_Cood && ball_X_Cood <= A_X_Cood + 10){//4번
            angle = 90;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(A_X_Cood + 10 <= ball_X_Cood && ball_X_Cood <= A_X_Cood + 30){//5번
            angle = 70;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(A_X_Cood + 30 <= ball_X_Cood && ball_X_Cood <= A_X_Cood + 50){//6번
            angle = 55;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(A_X_Cood + 50 <= ball_X_Cood && ball_X_Cood <= A_X_Cood + 70){//7번
            angle = 40;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }else if(ball_X_Cood - 36 <= A_X_Cood + 70){//7번
            angle = 40;
            setAngle(angle,speed);
            collision_check2 = 0;
            A_return = 1;
        }
    }else if(ball_X_Cood == A_X_Cood){//4번
        angle = 90;
        setAngle(angle,speed);
        collision_check2 = 0;
        A_return = 1;
...
void B_Collision_check(){
    if(ball_X_Cood < B_X_Cood){
        if(B_X_Cood - 10 <= ball_X_Cood && ball_X_Cood <= B_X_Cood){//4번
            angle = 270;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(B_X_Cood - 30 <= ball_X_Cood && ball_X_Cood <= B_X_Cood -10){//5번
            angle = 250;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(B_X_Cood - 50 <= ball_X_Cood && ball_X_Cood <= B_X_Cood -30){//6번
            angle = 235;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(B_X_Cood - 70 <= ball_X_Cood && ball_X_Cood <= B_X_Cood -50){//7번
            angle = 220;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        ...
        if(B_X_Cood <= ball_X_Cood && ball_X_Cood <= B_X_Cood + 10){//4번
            angle = 270;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(B_X_Cood + 10 <= ball_X_Cood && ball_X_Cood <= B_X_Cood + 30){//3번
            angle = 290;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(B_X_Cood + 30 <= ball_X_Cood && ball_X_Cood <= B_X_Cood + 50){//2번
            angle = 305;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(B_X_Cood + 50 <= ball_X_Cood && ball_X_Cood <= B_X_Cood + 70){//1번
            angle = 320;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
        }else if(ball_X_Cood - 36 <= B_X_Cood + 70){//1번
            angle = 320;
            setAngle(angle,speed);
            collision_check2 = 0;
            B_return = 1;
